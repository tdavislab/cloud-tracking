...
def partial_cg(a, b, M, reg, m, nb_dummies=1, G0=None, thres=1,
    ...
    while loop:
        ...
        M_emd[-nb_dummies:, -nb_dummies:] = np.max(Mi) * 1e3
        M_emd = np.asarray(M_emd, dtype=np.float64)

        Gcemd, logemd = emd(a_extended, b_extended, M_emd, emdIterMax, log=True)

        if logemd['warning'] is not None:
            if nb_dummies < 100:
                raise ValueError("Error in the EMD resolution: try to increase the"
                                " number of dummy points")
            elif prev_log_cost is not None and prev_log_cost - logemd['cost'] > eps:
                print("emd prev/current cost:", prev_log_cost, logemd['cost'])
                if emdIterMax < 300000:
                    emdIterMax += 100000
                else:
                    raise ValueError("Error in the EMD resolution: try to increase the"
                                " number of dummy points")
        prev_log_cost = logemd['cost']
