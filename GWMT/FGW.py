...
def fgw_partial_lp(M,C1,C2,p,q,m,alpha=1.0,amijo=True,G0=None,**kwargs):
    for nb_dummies in range(51, 302, 50):
        try:
            return optim.partial_cg(p, q, M, alpha, m, nb_dummies=nb_dummies, G0=G0, log=True, amijo=amijo, C1=C1, C2=C2, **kwargs)
        except ValueError as error_msg:
            print(error_msg)
            print("Trying to add 50 more dummy nodes. Current dummy node at {}.".format(str(nb_dummies)))
            continue
    raise optim.NonConvergenceError("Checking m={}".format(str(m)))
